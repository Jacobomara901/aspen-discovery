{strip}
	<div class="result row borrowbox_checkout_{$record->sourceId|escape}_{$record->userId}">
				...
				<div class="resultDetails col-xs-12 col-md-8 col-lg-9">
					...
					{if $record->checkoutDate}
						<div class="row">
							...
							<div class="col-sm-12 col-md-7 result-value">{$record->checkoutDate|format_date_locale}</div>
						</div>
					{/if}

					<div class="row">
						<div class="result-label col-sm-12 col-md-5">{translate text='Format' isPublicFacing=true}</div>
						<div class="col-sm-12 col-md-7 result-value">{implode subject=$record->getFormats() translate=true isPublicFacing=true} - {translate text="BorrowBox" isPublicFacing=true}</div>
					</div>
					...
					<div class="row">
						...
						<div class="col-sm-12 col-md-7 result-value">{$record->dueDate|format_date_locale}</div>
					</div>
				</div>
				...
				<div class="col-sm-12 col-md-4 col-lg-3">
					<div class="btn-group btn-group-vertical btn-block">
						{if !empty($record->accessOnlineUrl)}
							<a href="{$record->accessOnlineUrl}" target="_blank" class="btn btn-sm btn-action btn-wrap" aria-label="{translate text='Open in BorrowBox' isPublicFacing=true} ({translate text='opens in new window' isPublicFacing=true})"><i class="fas fa-external-link-alt" role="presentation"></i> {translate text='Open in BorrowBox' isPublicFacing=true}</a>
						{/if}
						{if $record->canRenew}
							<a href="#" onclick="return AspenDiscovery.BorrowBox.renewCheckout('{$record->userId}', '{$record->sourceId}');" class="btn btn-sm btn-info btn-wrap">{translate text='Renew Checkout' isPublicFacing=true}</a>
						{/if}
						{if $record->canReturnEarly}
							<a href="#" onclick="return AspenDiscovery.BorrowBox.returnCheckout('{$record->userId}', '{$record->sourceId}', '{$record->sourceId|escape}');" class="btn btn-sm btn-warning btn-wrap">{translate text="Return Now" isPublicFacing=true}</a>
						{/if}
					</div>
