				...
				<tbody>
					{foreach from=$logEntries item=logEntry}
						<tr>
							...
							<td>{$logEntry->startTime|format_date_locale:"short":"medium"}</td>
							<td>{$logEntry->lastUpdate|format_date_locale:"short":"medium"}</td>
							<td>{$logEntry->endTime|format_date_locale:"short":"medium"}</td>
							...
							<td><a href="#" onclick="return AspenDiscovery.Admin.showExtractNotes('{$logEntry->id}', 'borrowbox');">{translate text="Show Notes" isAdminFacing=true}</a></td>
						</tr>
					{/foreach}
				</tbody>
			</table>
