{include file="GroupedWork/load-full-record-view-enrichment.tpl" recordType="omekaRecord" recordId=$recordDriver->getId()}
			...
			<div class="col-xs-4 col-sm-5 col-md-4 col-lg-3 text-center">
				{if $disableCoverArt != 1}
					<div id="recordCover" class="text-center row">
						<a href="#" onclick="return AspenDiscovery.Omeka.getLargeCover('{$recordDriver->getUniqueID()}')"><img alt="{translate text='Book Cover' inAttribute=true isPublicFacing=true}" class="img-thumbnail{if $useOriginalCoverUrls} use-original-covers{/if} {$coverStyle}" src="{$recordDriver->getBookcoverUrl('medium')}" role="presentation"></a>
					</div>
				{/if}
			...
			<div id="main-content" class="col-xs-8 col-sm-7 col-md-8 col-lg-9">
				...
				<div class="row">
					<div id="record-details-column" class="col-xs-12 col-sm-9">
						{include file="Omeka/view-title-details.tpl"}
					</div>
							...
							<div class="btn-group btn-group-vertical btn-block">
								{foreach from=$actions item=curAction}
									{if !empty($curAction.url) && strlen($curAction.url) > 0}
										<a href="{$curAction.url}" {if !empty($curAction.target)}target="{$curAction.target}"{/if} class="btn btn-sm {if empty($curAction.btnType)}btn-action{else}{$curAction.btnType}{/if} btn-wrap" {if !empty($curAction.alt)}title="{translate text=$curAction.alt inAttribute=true isPublicFacing=true}"{/if}>{translate text=$curAction.title isPublicFacing=true}</a>
									{else}
