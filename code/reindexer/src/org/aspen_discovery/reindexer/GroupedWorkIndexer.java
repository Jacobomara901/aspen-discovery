...
public class GroupedWorkIndexer implements AutoCloseable {
	...
	private BorrowBoxRecordGrouper borrowBoxRecordGrouper;
	...
	private BorrowBoxProcessor borrowBoxProcessor;
	...
	public GroupedWorkIndexer(String serverName, Connection dbConn, Ini configIni, boolean fullReindex, boolean clearIndex, boolean regroupAllRecords, BaseIndexingLogEntry logEntry, Logger logger) {
		...
		borrowBoxProcessor = new BorrowBoxProcessor(this, dbConn, logger);
		borrowBoxRecordGrouper = new BorrowBoxRecordGrouper(dbConn, serverName, logEntry, logger);
	...
	TreeSet<String> borrowBoxRecordsSkipped = new TreeSet<>();
	...
	synchronized void processGroupedWork(Long id, String permanentId, String grouping_category, boolean allowRegrouping) throws SQLException {
		...
		if ((regroupAllRecords && allowRegrouping) || permanentId.endsWith("|||") || permanentId.contains(" ")){
			...
			for (RecordIdentifier recordIdentifier : recordIdentifiers) {
				...
				} else if (type.equals("borrowbox")) {
					newId = borrowBoxRecordGrouper.processBorrowBoxRecord(identifier);
				}
				if (newId == null) {
	...
	private void updateGroupedWorkForPrimaryIdentifier(AbstractGroupedWorkSolr groupedWork, String type, String identifier)  {
		...
		switch (type) {
			...
			case "palace_project":
				...
				break;
			case "borrowbox":
				borrowBoxProcessor.processRecord(groupedWork, identifier, logEntry);
				break;
			default:
